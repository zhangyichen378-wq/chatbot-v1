...
def ask_math(question):
    ...
    try:
        ...
        if user_answer == answer:
            ...
            print(f"MiniGPT: Wrong! ❌ The correct answer is {answer}")
    except:
        print("Mini-ChatGPT: Invalid math expression!")
...
def random_math_quiz():
    ...
    op = random.choice(["+", "-", "*"])
    question = f"{a}{op}{b}"
    print(f"Mini-ChatGPT: Solve this: {question}")
    ask_math(question)
...
while True:
    ...
    if "my name is" in user_input:
        ...
        print(f"Mini-ChatGPT: Nice to meet you, {user_name}!")
        continue


    if "time" in user_input:
        current_time = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"Mini-ChatGPT: The current time is {current_time}")
        continue
    ...
    for key in responses:
        if key in user_input:
            print("Mini-ChatGPT:", random.choice(responses[key]))
            response_found = True
            break

    if not response_found:
        print("Mini-ChatGPT: Sorry, I don't understand that 😅")
